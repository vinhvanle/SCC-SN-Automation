 ...
 */
Given(
  /(.*): As (a|an) "(.*)" user, I login to SN Dev Instance$/,
  async function (testid, prefixTxt, user) {
    reporter.addStep(
      this.testid,
      "info",
      `Starting to login to SN Dev Instance`
    );
    try {
      // navigate to SN
      ...
      reporter.addStep(
        this.testid,
        "info",
        `>> Logged in as ${user} user, username: ${username} successful`
      );
    } catch (e) {
    ...
    reporter.addStep(
      this.testid,
      "info",
      `Login to SN Dev Instance successful`
    );
  }
);
